import "./UserItem.css";
import Card from "react-bootstrap/Card";
import ListGroup from "react-bootstrap/ListGroup";
import { Link } from "react-router-dom";

function UserItem(props) {

    

  return (
    <>
      <Card style={{ width: "18rem" }}>
        <Card.Img
          variant="top"
          src="data:image/png;base64,"
        />
       
        <ListGroup className="list-group-flush">
          <ListGroup.Item>نام : {props.name}</ListGroup.Item>
          <ListGroup.Item>نام خوانوادگی : {props.family}</ListGroup.Item>
          <ListGroup.Item>کد ملی : {props.national_code}</ListGroup.Item>
          <ListGroup.Item>علاقه مندی : {props.favorite}</ListGroup.Item>
        </ListGroup>
        <Card.Body>
          <Link to={`/user/${props.id}`} className='link'>مشاهده بیشتر</Link>
        </Card.Body>
      </Card>
    </>
  );
}

export default UserItem;
